...
Run the program with the `-bruteforce` flag and a sample packet. The program will try all game server access keys from `00000000` to `ffffffff` against the sample packet. The program will exit once a valid game server access key has been found. This process is split into `-threads` number of goroutines, defaulting to `runtime.NumCPU()`. Setting this higher than your CPU core count may result in slowdowns.

This mode can be very slow depending on the title (some may have valid access keys very close to the end of the uint32 space) and on your system details (slower systems/less CPU cores will take longer). In a test running on a 2024 Mac Mini with a 14 CPU core M4 Pro chip, it took around 2 minutes to find the Terraria game server access key using the CPU backend. Use the `-gpu` flag to run the extraction on the GPU for significantly faster results with modern GPUs. In a test running on a 2024 Mac Mini with a 20 GPU core M4 Pro chip, it took around 5 seconds to find the Terraria game server access key, and 0.2 seconds to find a valid Mario Kart 7 game server access key, using the Metal backend.

When bruteforcing a PRUDPv1 game server access key, the program should only find 1 valid key; the original. When bruteforcing a PRUDPv0 game server access key, the program will find thousands of valid keys, and will return only the first one. It is impossible to determine which key is the original for PRUDPv0 packets in bruteforce mode.
...
| `-gpu`             | Optional. If set the program will attempt to run the extraction on the GPU. Automatically selects the available GPU backend, defaulting back to the CPU if one cannot be found. Using the GPU will have significantly faster results with modern GPUs.         |
